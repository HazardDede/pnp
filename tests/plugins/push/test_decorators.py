...
def test_enveloped_for_doc(push_enveloped):
    assert push_enveloped._push.__doc__ == "Push, push!"
...
async def test_enveloped_for_correct_split(push_enveloped):
    push = push_enveloped
    assert push._push({}) == ({}, {})
    assert push._push(None) == ({}, None)
    assert push._push({'data': 'value'}) == ({}, 'value')
    assert push._push({'payload': 'value'}) == ({}, 'value')
    assert push._push({'key': 'any'}) == ({}, {'key': 'any'})
    assert push._push({'data': 'value', 'header': 'head'}) == ({'header': 'head'}, 'value')
...
def test_parse_envelope_for_doc():
    ...
    assert _Dummy(name='pytest')._push.__doc__ == "Push, push!"
...
async def test_parse_envelope():
    class _Dummy(SyncPush):
        ...
        @parse_envelope('key1')
        @parse_envelope('key2')
        def _push(self, key1, key2, envelope=None, payload=None):
            """Push, push!"""
    ...
    assert dut._push(None, None) == (None, None, None, None)
    assert dut._push({}, {}) == (None, None, {}, {})
    assert dut._push({}, {'key': 'value'}) == (None, None, {}, {'key': 'value'})
    assert dut._push({'anykey': 'anyval'}, {}) == (None, None, {'anykey': 'anyval'}, {})
    assert dut._push({'key1': 'value1'}, {}) == ('value1', None, {'key1': 'value1'}, {})
    assert (dut._push({'key1': 'value1', 'key2': 'value2'}, {})
            == ('value1', 'value2', {'key1': 'value1', 'key2': 'value2'}, {}))
    assert (dut._push({'key1': 'value1'}, 'payload')
...
def test_parse_envelope_with_parser():
    class _Dummy(SyncPush):
        ...
        @parse_envelope('key1')
        def _push(self, key1, envelope=None, payload=None):
            """Push, push!"""
            return key1, envelope, payload

    dut = _Dummy(name='pytest')
    assert (dut._push({'key1': 'val1'}, 'datadata')
...
def test_parse_envelope_with_instance_var():
    ...
    dut = _Dummy(name='pytest')
    assert (dut._push(None, 'datadata')
...
async def test_enveloped_parse_envelope_combination():
    ...
    dut = _Dummy(name='pytest')
    assert (dut._push({'data': 'datadata', 'key1': 'val1'})
            == ('val1', None, {'key1': 'val1'}, 'datadata'))
    assert (dut._push({'data': 'datadata', 'key1': 'val1', 'key2': 'val2'})
            == ('val1', 'val2', {'key1': 'val1', 'key2': 'val2'}, 'datadata'))
    assert (dut._push({'data': 'datadata', 'anykey': 'anyval'})
...
async def test_drop_envelope():
    ...
    dut = _Dummy(name='pytest')
    assert dut._push({'data': 'datadata', 'key1': 'val1'}) == 'datadata'
