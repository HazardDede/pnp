...
async def http_call(
        mp, call_method='GET', call_url='http://localhost:1234/foo/bar', call_data='payload',
    ...
    return await dut.push(call_data)
...
async def test_call_with_error(monkeypatch):
    ...
    with pytest.raises(
            PushExecutionError,
            match="POST of 'http://any_host:3546/foo/bar' failed with status code = '500'"
    ):
        await http_call(
            monkeypatch, call_method='POST', call_url='http://any_host:3546/foo/bar', status_code=500,
            fail_on_error=True
        )
...
async def test_call_with_envelope_override(monkeypatch):
    ...
    res = await http_call(monkeypatch, call_data=dict(data="payload", method="PUT"), call_assert_fun=assert_put_method)
    ...
    def assert_url(**kwargs):
        ...
        assert kwargs.pop('data') == "payload"
    res = await http_call(
        monkeypatch, call_data=dict(data="payload", url="http://another_host:9876/bar/baz"),
    ...
    with pytest.raises(
        PushExecutionError,
        match="GET of 'http://localhost:1234/foo/bar' failed with status code = '500'"
    ):
        await http_call(
