...
@pytest.mark.asyncio
async def test_push_with_file(mocker):
    dbx_mock = setup_mock(mocker.patch('pnp.plugins.push.storage.dropbox.dropbox.Dropbox'))
...
@pytest.mark.asyncio
async def test_push_with_file_and_shared_link(mocker):
    dbx_mock = setup_mock(mocker.patch('pnp.plugins.push.storage.dropbox.dropbox.Dropbox'))
...
def test_repr():
    dut = Dropbox(name='pytest', api_key='secret', target_file_name="42", create_shared_link=True)
    assert repr(dut) == "Dropbox(create_shared_link=True, name='pytest', target_file_name='42')"
